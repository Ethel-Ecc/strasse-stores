...
def registration_store_blog():
    if request.method == 'POST':
        ...
        if error is None:
            connect_db.execute('INSERT INTO users_store_blog (username, password) VALUES (?, ?)',(username, generate_password_hash(password)))
            connect_db.commit()
...
def load_logged_in_user_sessions():
    ...
    if logged_in_user_id is None:
        g.logged_in_user_id = None
    else:
        g.logged_in_user_id = connect_to_store_blog_db().execute(
